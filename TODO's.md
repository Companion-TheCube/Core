...
- [CMakeLists.txt:7] add build targets for debug and release. release target needs to set a build define for PRODUCTION_BUILD.
- [get_next_build_number.py:63] need to upload the executable file to the build info server so that we can pull it later on in case we need a specific build.
- [tests/apps/appsManager.test.cpp:1] (no description)
- [tests/apps/dockerApi.test.cpp:1] (no description)
- [tests/logger/logger.test.cpp:33] set up mocking for file logging
- [tests/logger/logger.test.cpp:50] Check if the output matches the expected result    
- [tests/settings/loader.test.cpp:1] (no description)
- [tests/api/builder.test.cpp:1] (no description)
- [tests/api/authentication.test.cpp:1] (no description)
- [tests/api/api.test.cpp:1] (no description)
...
- [src/audio/speechIn.cpp:205] this needs to happen in another thread so that we don't block the audio input thread
- [src/audio/speechIn.cpp:248] implement silence detection to determine when the user is done speaking.
- [src/gui/eventHandler/eventHandler.cpp:36] Need to refactor so that the functions that take in "void* data" also take
- [src/gui/characterManager.cpp:36] Character manager needs some static methods that handle changing / triggering animations and expressions.
- [src/gui/characterManager.cpp:38] scratch that todo above. we'll provide api endpoints for this stuff.
- [src/gui/characterManager.cpp:40] As it is, all the characters are loaded regardless of whether they are used or not. We need to make it so that
- [src/gui/characterManager.cpp:141] This will need to interface with the list of registered apps and find the ones
- [src/gui/characterManager.cpp:236] fill in the endpoints
- [src/gui/characterManager.cpp:299] move the actual loading of character data into a separate method so that we can call it
- [src/gui/characterManager.cpp:301] also need way to unload a character.
...
- [src/gui/renderables/theCube._cpp:110] Implement bounce animation. Return true when animation is complete.
- [src/gui/renderables/theCube._cpp:153] Implement expand animation. Return true when animation is complete.
- [src/gui/renderables/theCube._cpp:157] Implement jump animation. Return true when animation is complete.
- [src/gui/renderables/theCube._cpp:161] Implement shrink animation. Return true when animation is complete.
- [src/gui/renderables/theCube._cpp:165] Implement spin animation. Return true when animation is complete.
...
- [src/gui/messageBox/messageBox.cpp:198] (no description)
- [src/gui/messageBox/messageBox.cpp:208] (no description)
...
- [src/gui/gui.cpp:299] add a text box to enter the SSID
- [src/gui/gui.cpp:309] add a dropdown to select the security type
- [src/gui/gui.cpp:319] add a text box to enter the password
- [src/gui/gui.cpp:333] connect to the network
- [src/gui/gui.cpp:354] Trigger the wif manager to scan for networks and provide a way for it to return the list of networks.
- [src/gui/gui.cpp:376] list all the known networks
- [src/gui/gui.cpp:395] get the MAC address of the wifi adapter
- [src/gui/gui.cpp:411] get the IP address of the wifi adapter
- [src/gui/gui.cpp:427] get the subnet mask of the wifi adapter
- [src/gui/gui.cpp:443] get the signal strength of the wifi adapter
- [src/gui/gui.cpp:463] get the name of the current network
- [src/gui/gui.cpp:479] get the type of the current network
- [src/gui/gui.cpp:495] get the security type of the current network
- [src/gui/gui.cpp:511] get the frequency of the current network
- [src/gui/gui.cpp:527] get the channel of the current network
- [src/gui/gui.cpp:543] get the BSSID of the current network
- [src/gui/gui.cpp:559] get the gateway of the current network
- [src/gui/gui.cpp:575] get the DNS servers of the current network
- [src/gui/gui.cpp:591] get the DHCP server of the current network
- [src/gui/gui.cpp:607] get the lease time of the current network
- [src/gui/gui.cpp:623] get the connection time of the current network
- [src/gui/gui.cpp:639] get the data rate of the current network
- [src/gui/gui.cpp:659] when pairing mode is clicked, bluetooth should go into pairing mode and show a list of devices that can be paired with.
- [src/gui/gui.cpp:662] list all the bluetooth devices that have been paired with the cube
- [src/gui/gui.cpp:681] make this actually enable/disable NFC. the settings should have a callback registered with the GlobalSettings class that will enable/disable NFC when the setting is changed.
- [src/gui/gui.cpp:686] return GlobalSettings::getSettingOfType<bool>(GlobalSettings::SettingType::NFC_ENABLED);
- [src/gui/gui.cpp:698] show a fullscreen message box with information about NFC
- [src/gui/gui.cpp:719] make this actually enable/disable personality. the settings should have a callback registered with the GlobalSettings class that will enable/disable personality when the setting is changed.
- [src/gui/gui.cpp:732] reset the personality to default
- [src/gui/gui.cpp:757] set the curiosity level
- [src/gui/gui.cpp:762] before adding the rest of these, we need to figure out the slider rendering and how to get/set the values
- [src/gui/gui.cpp:786] make this actually enable/disable the microphone. the settings should have a callback registered with the GlobalSettings class that will enable/disable the microphone when the setting is changed.
...
- [src/gui/gui.cpp:1000] make this actually enable/disable remote animations. the settings should have a callback registered with the GlobalSettings class that will enable/disable remote animations when the setting is changed.
- [src/gui/gui.cpp:1017] list all the idle animations
- [src/gui/gui.cpp:1033] set the brightness
- [src/gui/gui.cpp:1039] save the brightness to the settings
- [src/gui/gui.cpp:1058] make this actually enable/disable auto off. the settings should have a callback registered with the GlobalSettings class that will enable/disable auto off when the setting is changed.
- [src/gui/gui.cpp:1074] set the auto off time
- [src/gui/gui.cpp:1084] save the auto off time to the settings
- [src/gui/gui.cpp:1101] list all the available fonts
- [src/gui/gui.cpp:1104]         // the settings should have a callback registered (TODO:) with the GlobalSettings class that will set the font when the setting is changed.
- [src/gui/gui.cpp:1120] figure out what to put here: list apps that use microphone, presence detection, etc. and allow the user to disable them
- [src/gui/gui.cpp:1133] list all the accounts that have been added to the system. These are stored in the database.
- [src/gui/gui.cpp:1136] (no description)
- [src/gui/gui.cpp:1139] (no description)
- [src/gui/gui.cpp:1142] add a button to add an account
- [src/gui/gui.cpp:1157] (no description)
- [src/gui/gui.cpp:1160] for each installed app do the following
- [src/gui/gui.cpp:1165] each app will fill in the details here
- [src/gui/gui.cpp:1168] (no description)
- [src/gui/gui.cpp:1171] List all the available apps here. each app gets an install button and a details button
- [src/gui/gui.cpp:1206]         ///////// General Settings Menu - Date and Time - Set Time Zone ///////// TODO:
- [src/gui/gui.cpp:1207]         ///////// General Settings Menu - Date and Time - Set Time Format ///////// TODO:
- [src/gui/gui.cpp:1208]         ///////// General Settings Menu - Date and Time - Set Date Format ///////// TODO:
- [src/gui/gui.cpp:1209]         ///////// General Settings Menu - Date and Time - Enable Automatic Date and Time ///////// TODO:
- [src/gui/gui.cpp:1216] (no description)
- [src/gui/gui.cpp:1218] All the strings in the system should be in a language file that can be changed at runtime.
- [src/gui/gui.cpp:1230] figure out what should go here
- [src/gui/gui.cpp:1237] (no description)
- [src/gui/gui.cpp:1239] (no description)
- [src/gui/gui.cpp:1243] (no description)
- [src/gui/gui.cpp:1260] get the serial number from the hardware class
- [src/gui/gui.cpp:1302] Show the version, build number, and build date of TheCube-CORE
- [src/gui/gui.cpp:1304] Show the Raspbian version, kernel version, etc
- [src/gui/gui.cpp:1306] Show the versions of all the libraries used in the system
- [src/gui/gui.cpp:1313] (no description)
- [src/gui/gui.cpp:1316] (no description)
- [src/gui/gui.cpp:1319] (no description)
- [src/gui/gui.cpp:1322] (no description)
- [src/gui/gui.cpp:1325] (no description)
- [src/gui/gui.cpp:1328] (no description)
- [src/gui/gui.cpp:1331] (no description)
- [src/gui/gui.cpp:1334] (no description)
- [src/gui/gui.cpp:1337] (no description)
- [src/gui/gui.cpp:1340] (no description)
- [src/gui/gui.cpp:1343] list all the open source licenses used in the system
- [src/gui/gui.cpp:1346] list all the legal notices
- [src/gui/gui.cpp:1349] list all the trademarks used in the system
- [src/gui/gui.cpp:1352] Read in the EULA from a file and display it
- [src/gui/gui.cpp:1355] Read in the privacy policy from a file and display it
- [src/gui/gui.cpp:1393] call some method that checks the SSD integrity
- [src/gui/gui.cpp:1413] call some method that resets the system to factory settings
- [src/gui/gui.cpp:1430] trigger a reboot
- [src/gui/gui.cpp:1447] trigger a shutdown
- [src/gui/gui.cpp:1459] (no description)
- [src/gui/gui.cpp:1462] (no description)
- [src/gui/gui.cpp:1471] (no description)
- [src/gui/gui.cpp:1474] (no description)
- [src/gui/gui.cpp:1477] (no description)
- [src/gui/gui.cpp:1480] (no description)
...
- [src/gui/gui.cpp:1828] show a text input box
- [src/gui/gui.cpp:1845] Add the following endpoints:
- [src/gui/gui.cpp:1855] anything that gets displayed needs to be logged in the DB->notifications
- [src/gui/gui.cpp:1879] anything that gets displayed needs to be logged in the DB->notifications
- [src/gui/gui.cpp:2750] Implement
- [src/gui/statusBar.h:37] (no description)
...
- [src/decisionEngine/remoteServer.cpp:43] (no description)
- [src/decisionEngine/remoteServer.cpp:58] Read the setting for which AI service the user wants to use.
- [src/decisionEngine/remoteServer.cpp:85] (no description)
- [src/decisionEngine/remoteServer.cpp:92] (no description)
- [src/decisionEngine/remoteServer.cpp:99] (no description)
- [src/decisionEngine/remoteServer.cpp:196] (no description)
...
- [src/decisionEngine/decisions.cpp:80] (no description)
...
- [src/decisionEngine/decisions.cpp:423] (no description)
...
- [src/decisionEngine/decisions.cpp:590]     return false; // TODO:
- [src/decisionEngine/decisions.cpp:597]     return nullptr; // TODO:
- [src/decisionEngine/decisions.cpp:612] remove this. Testing only.
- [src/decisionEngine/decisions.cpp:667] (no description)
...
- [src/decisionEngine/decisions.cpp:722] (no description)
...
- [src/decisionEngine/decisions.cpp:907] (no description)
- [src/decisionEngine/decisions.cpp:920] (no description)
...
- [src/main.cpp:265] All the base apps should be inserted into the database and/or verified in the database here.
- [src/main.cpp:266] Add the openwakeword python script to the DB here. This will be a native app and will use the python executable in the openwakeword/bin directory.
...
- [src/main.cpp:340] any other place where main() might return, change the return value to something meaningful. this way, when
- [src/main.cpp:345] this probably needs a mutex for breakMain
...
- [src/hardware/bluetooth.cpp:572]     // this->server->Get("/scan_started") // TODO:
