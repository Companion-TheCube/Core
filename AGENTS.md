...
## Code Reuse and Abstraction Guidelines

* Always prefer reusing existing classes, functions, and modules before introducing new implementations.
* When adding new functionality, actively search the codebase for similar or overlapping logic that can be extended or reused.
* If direct reuse is not possible, evaluate overlap with existing implementations:

  * Identify duplicated or near-duplicated logic.
  * Determine whether the existing implementation can be generalized.
* When overlap is identified and abstraction is beneficial:

  * Refactor into a more generic, reusable class, function, or utility.
  * Ensure backward compatibility with existing code where reasonable.
  * Avoid premature abstraction; ensure the generalization has clear reuse value.
* Prefer composability over duplication. Favor small, reusable building blocks.
* Document any new abstractions clearly so future contributors understand intended reuse patterns.
